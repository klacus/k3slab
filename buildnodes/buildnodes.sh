...
for NEWNODE in "${K3SSERVERHOSTS[@]}"; do
  ...
  ./newk3snode.sh $NEWNODE ${VMTEMPLATEFILE} ${NEWVMFILE} ${OSVMGROUP} ${SERVERRAM} ${SERVERCPU} ${VMOSVARIANT} ${VMNETWORKBRIDGE} ${VMNETWORKNAME} ${NEWVMMAC} ${VMGRAPHICS} ${SSHKEYFILE} ${VMUSER} ${TEMPLATEHOSTNAME}
...
for NEWNODE in "${K3SAGENTHOSTS[@]}"; do
  ...
  ./newk3snode.sh $NEWNODE ${VMTEMPLATEFILE} ${NEWVMFILE} ${OSVMGROUP} ${AGENTRAM} ${AGENTCPU} ${VMOSVARIANT} ${VMNETWORKBRIDGE} ${VMNETWORKNAME} ${NEWVMMAC} ${VMGRAPHICS} ${SSHKEYFILE} ${VMUSER} ${TEMPLATEHOSTNAME}
